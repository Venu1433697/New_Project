pipeline{
  ...
  stages
  {
    ...
    stage('Deploy through Jenkins'){
      steps{
        withCredentials([sshUserPrivateKey(credentialsId: 'New_Project', keyFileVariable: 'SSH_KEY', usernameVariable: 'SSH_USER' )]){
            bat '''
              ...
              ssh -i $SSH_KEY $SSH_USER@$SSH_HOST  'sudo systemctl restart httpd'
            '''
        }
      }
    }
